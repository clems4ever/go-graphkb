...
func buildSQLConstraintsFromPatterns(queryGraph *QueryGraph, constrainedNodes map[int]bool, scope Scope) ([][]SQLJoin, []SQLFrom, error) {
	from := []SQLFrom{}
	relationSet := make(map[*QueryRelation]struct{ ralias string })
				...
				if relation.LeftIdx == i {
					...
					if relationToAssetExists {
						processedRelations = append(processedRelations, struct {
							processedRelation      *QueryRelation
							processedRelationAlias string
						}{processedRelation: relation, processedRelationAlias: relationSetStruct.ralias})
					}
				} else {
		...
		for j, relation := range relations {
			...
			if relation.Direction == Right && relation.LeftIdx == i {
				...
				if !optimize { // if this relation is optimizable, then just on direction is needed as (v) -- (q) <==> (q) -- (v)
					...
					relationsCopy := make([]QueryRelation, len(queryGraph.Relations))
					nodesCopy := make([]QueryNode, len(queryGraph.Nodes))

					copy(relationsCopy, queryGraph.Relations)
					copy(nodesCopy, queryGraph.Nodes)

					queryGraphClone := QueryGraph{
						Nodes:          nodesCopy,
						Relations:      relationsCopy,
						VariablesIndex: queryGraph.VariablesIndex,
					}
			...
			relationSet[relation] = struct{ ralias string }{ralias: ralias}
